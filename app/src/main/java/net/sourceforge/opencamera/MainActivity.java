...
public class MainActivity extends Activity implements AudioListener.AudioListenerCallback {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(() -> textToSpeech = new TextToSpeech(MainActivity.this, new TextToSpeech.OnInitListener() {
			@Override
	...
	public void onAudio(int level) {
		...
		if( audio_trigger ) {
			...
			if( time_last_audio_trigger_photo != -1 && time_now - time_last_audio_trigger_photo < 5000 ) {
				...
				try{audioTrigger();}catch (FileNotFoundException e){e.printStackTrace();}
	 ...
	 */
	private void audioTrigger() throws FileNotFoundException {
		if( MyDebug.LOG )
		...
		if( popupIsOpen() ) {
			...
			this.runOnUiThread(new Runnable() {
				public void run() {
					...
					try{takePicture();}catch (FileNotFoundException e){e.printStackTrace();}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)  {
		...
		if( handled )
			return true;
		}catch (FileNotFoundException e){ Log.d(TAG, "onKeyDown: "+e.getMessage()); }
    ...
    public void clickedTakePhoto(View view) throws FileNotFoundException {
		if( MyDebug.LOG )
    ...
    public void clickedSwitchCamera(View view) throws FileNotFoundException {
		if( MyDebug.LOG )
    ...
    public void clickedSwitchVideo(View view) throws FileNotFoundException {
		if( MyDebug.LOG )
    ...
    public void clickedSettings(View view) throws FileNotFoundException {
		if( MyDebug.LOG )
    ...
    public void openSettings() throws FileNotFoundException{
		if( MyDebug.LOG )
    ...
    public void updateForSettings() throws FileNotFoundException {
	 ...
	 */
	public void updateForSettings(String toast_message) throws FileNotFoundException {
		if( MyDebug.LOG ) {
    ...
    void takePicturePressed() throws FileNotFoundException{
		if( MyDebug.LOG )
		...
		switch( requestCode ) {
	        case MY_PERMISSIONS_REQUEST_CAMERA:
	        {
	            ...
	            if( grantResults.length > 0
	        		...
	        		try{preview.retryOpenCamera();} catch (FileNotFoundException e) {
						e.printStackTrace();
					}
